import { SWF } from '../../constants'
var swfobject = require('./better.swfobject')
var _ = require('underscore')
var Observable = require('../../mixins/Observable')

var Swf = function Swf(config) {
...
Swf.prototype = {
  ...
  // Flash adapter is not allowed until we can export
  // the wave blob via onStoppedRecording
  isAllowed() {
    return false
  },
  ...
  stopRecording() {
    if (this._recording) {
      this._recording = false
      this.flashInterface().recordStop()
      // @see branch: flash_onstopped_wav
      // this.notifyObservers('onStoppedRecording', this.flashInterface().wavData())
