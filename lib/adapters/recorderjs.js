...
RecorderJsAdapter.prototype = {
  ...
  startRecording: function() {
    this.ensureRecorder().then(_.bind(function(recorder) {
      this.stopRecordingHandler(recorder);
      recorder.record();
  ...
  startPlaying: function() {
    ...
    this.stopPlayingHandler(source);
    source.onended = this.stopPlayingHandler(source);
  ...
  stopRecordingHandler: function(recorder) {
    this.stopRecording = _.once(_.bind(function(recorder) {
      recorder.stop();
  ...
  stopPlayingHandler: function(source) {
    this.stopPlaying = _.once(_.bind(function(source) {
      source.stop();
